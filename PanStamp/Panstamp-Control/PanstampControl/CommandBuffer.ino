...
void CommandBuffer::Fill(char* s)
{
  commandPointer = 0;
  while(s[commandPointer] && (s[commandPointer] != ';'))
  {
    if(commandPointer >= COMMAND_LENGTH)
    {
      Serial.print("CommandBuffer.Fill(): length overflow.\n");
      commandBuffer[0] = 0;
      Init();
      return;
    }
    commandBuffer[commandPointer] = s[commandPointer];
    commandPointer++;
  }
...
bool CommandBuffer::Put(char c)
{
  ...
  if(commandPointer >= COMMAND_LENGTH)
  {
    Serial.print("CommandBuffer.Put(): length overflow.\n");
    commandPointer = 0;
    commandBuffer[0] = 0;
  }
...
float CommandBuffer::GetFValue()
{
  if(readPointer < 0)
  {
     message->Say("Command: Attempt to read a GCode float before a search.\n");
     return 0.0;
  }
...
char* CommandBuffer::GetString()
{
  if(readPointer < 0)
  {
      message->Say("Command: Attempt to read a string before a search.\n");
...
long CommandBuffer::GetLValue()
{
  if(readPointer < 0)
  {
    message->Say("Command: Attempt to read a command long before a search.\n");
    return 0;
  }
